const data = {
    ...
    getdp: {
        ...
        EXTENDED_DESCRIPTION: "```Get the profile picture of the group in a group conversation or the profile picture of 💠DarkKnight's Bot💠 itself in personal chat. Tag a person in a group to get their profile picture.```",
        IMAGE_CAPTION: "```Here is the display image. Procured by 💠DarkKnight's Bot💠.```",
    ...
    help: {
        ...
        HEAD: "🌀 *DarkKnight's Bot command list* 🌀\n```Use .help command for detailed info on a module.```",
        TEMPLATE: "\n\n🤖 *Command* - ```{}```\n💡 *Info* - ```{}```",
        COMMAND_INTERFACE: "🌀 *DarkKnight's Bot Command Interface* 🌀\n\n",
    ...
    tr: {
        ...
        EXTENDED_DESCRIPTION: "```Usedark```  *.tr <text> | <language>*  ```to translate text to the specified language. You can also reply to a text message with syntax```  *.tr <language>*  ```to translate text.\nIf you do not specify a language, it defaults to <English class=''></English>```",
